$(document).ready(function() {
	...
	$('#drop-files').bind('drop', function(e) {
		...
		$.each(files, function(index, file) {
			...
			if (!files[index].type.match('image.*')) {
				...
				if(errMessage == 0) {
					$('#drop-files').html('Hey! Solo Imágenes');
					++errMessage
				}
				else if(errMessage == 1) {
					$('#drop-files').html('Detente! Solo Imágenes!');
					++errMessage
				}
				else if(errMessage == 2) {
					$('#drop-files').html("No sabe leer?! Solo Imágenes!");
					++errMessage
				}
				else if(errMessage == 3) {
					$('#drop-files').html("Bien! Sigue subiendo archivos que no son Imágenes.");
					errMessage = 0;
				}
				return false;
			}
				...
				fileReader.onload = (function(file) {
					...
					return function(e) { 
						...
						if(dataArray.length == 1) {
							$('#upload-button span').html("1 Eeperando a ser subido");
						} else {
							$('#upload-button span').html(dataArray.length+" archivos esperando a ser subidos");
						}
	...
	$('#upload-button .upload').click(function() {
		...
		$.each(dataArray, function(index, file) {	
			...
			$.post('q/upload_imagen_proceso.php', dataArray[index], function(data) {
				...
				if(totalPercent*(x) == 100) {
					...
					$('#loading-content').html('Subida Completa!');
				...
				if(dataSplit[1] == 'Archivos subidos con exito') {
					var realData = '<li><a href="images/'+dataSplit[0]+'">'+fileName+'</a> '+dataSplit[1]+'</li>';
					
					$('#uploaded-files').append('<li><a href="../upload_imagenes/'+dataSplit[0]+'">'+fileName+'</a> '+dataSplit[1]+'</li>');
					...
					$('#uploaded-files').append('<li><a href="../upload_imagenes/'+data+'. File Name: '+dataArray[index].name+'</li>');
				}
	...
	$('#drop-files').bind('dragenter', function() {
		$(this).css({'box-shadow' : 'inset 0px 0px 20px rgba(0, 0, 0, 0.1)', 'border' : '4px dashed #009900'});
		return false;
	});
